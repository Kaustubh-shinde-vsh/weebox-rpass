...
var accessCodeCheckbox;
var loginState = mpaasConstants.loginState;
var mpassTextCheck = false;
function init() {
  console.log("a1- ");
  if ($("input#assessment_name_input").length === 1) {
    return;
  }
  console.log("a2- ");
  $("#yesRadio").prop("disabled", true);
  displayMpaasTemplate();
  inputAccessCode = $("input#fPassword");
  accessCodeCheckbox = $("input#fIsPassword");
  switchBox = $("#switch");
  saveBtn = $("#bottom_Submit");
  bindEvents();
}
function displayMpaasTemplate() {
  ...
  let quizMode = url.includes("modifyOptions");
  if (quizMode) {
    let editQuiz = {
      course: encodeURIComponent(localStorage.getItem("courseName")),
      quiz: encodeURIComponent($("input#name").val()),
...
function mpassOptionsSuccess(a) {
  ...
  if (a.proctoringEnabled) {
    ...
    let access = JSON.parse(a.accessCode);
    let disabledNote =
    ...
    $("#help_fPassword").append(disabledNote);
    let SaveMpaasNote =
    ...
    $("#help_fPassword").append(SaveMpaasNote);
    let notSupportChar =
    ...
    $("#step1").append(notSupportChar);
    inputAccessCode.val().length > 0 ? enableSaveBtn() : disableSaveBtn();
    $(".mpaas-txt").show();
    $("input#fPassword").val(decodeURIComponent(access.accessCode));
    $("#mpaas").show();
...
function prepareSaveQuizData() {
  return {
    ...
    mPassPayload: {
      ...
      accessCode: encodeURIComponent($("input#fPassword").val()),
...
function checkCourseSpecialChar(c) {
  ...
  if (b(c) == false) {
    checkQuizSpecialChar($("input#name").val());
  } else {
    let error =
      "Please remove the special characters from the name of your Course";
    $("#step1").append(sharedTemplate.mpaaSErrorPopup(error));
...
function checkQuizSpecialChar(c) {
  ...
  if (a(c) == false) {
    $("#step1").append(sharedTemplate.mpaaSSavePopup());
...
function saveQuizData() {
  ...
  if (a.mPassPayload.proctoringEnabled) {
    if ($("input#fPassword").val().length <= 0) {
  ...
  apiService.saveQuiz(a).then(
    ...
    function (b) {
      if (switchBox.is(":checked")) {
        $("#step1").append(sharedTemplate.mpaaSErrorPopup(b));
        $("#mPaaSErrorPopup").show();
      } else {
...
function enableSaveBtn() {
  $("#bottom_Submit").removeAttr("disabled");
  $(".note-txt").hide();
}
function disableSaveBtn() {
  $("#bottom_Submit").prop("disabled", true);
  $(".note-txt").show();
}
...
function bindEvents() {
  inputAccessCode.keyup(function () {
    ...
    if (switchBox.is(":checked") && accessCodeCheckbox.is(":checked")) {
  ...
  switchBox.change(function () {
    if (switchBox.is(":checked")) {
      ...
      $("#mpaas").show();
      let disabledNote =
        "<div class='col-sm-12 note-section text-center my-2'><h5 class='note-txt text-black text-center'>Note: Please set a password for save quiz</h5></div><div class='mpaas-txt text-black text-center'><h5>RPaaS Chrome Extension link has been added in the Quiz Description.</h5></div>";
      $("#help_fPassword").prepend(disabledNote);
      let SaveMpaasNote =
        '<span class="mpaas-txt info-notetxt"><img src=' +
        chrome.runtime.getURL("assets/info.png") +
        ' alt="info"><div>Note: Please do not change the name and code of the Course and Quiz after saving the settings. If you do so, RPaaS will get reset.</div></span>';
      $("#help_fPassword").prepend(SaveMpaasNote);
      let notSupportChar =
        '<span class="mpaas-txt warning-notetxt" ><img src=' +
        chrome.runtime.getURL("assets/info.png") +
        ' alt="info"><div>Warning: Please make sure your Course and Quiz name do not have the following special characters: <b> & % ; < > " &#92; &#47; </b></div></span>';
      $("#step1").append(notSupportChar);
      mpassTextCheck = true;
      accessCodeCheckbox.is(":checked") ? enableSaveBtn() : disableSaveBtn();
    } else {
      $("#descriptiontext_ifr")
        .contents()
        .find("#tinymce")
